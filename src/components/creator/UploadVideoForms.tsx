import { Button, FormLabel, ModalBody, ModalContent, ModalFooter, chakra, useBoolean } from "@chakra-ui/react";
...
export const UploadVideoForms = ({ onClose, id }: Props) => {
  ...
  const { data, error } = id ? useSWR(`api/supabase/getVideo/${id}`, fetcherDefault) : { data: null, error: null };
    ...
    try {
      if (videoId) {
        const { error } = await supabaseClient
          .from("videos")
          .update({ title: data.title, description: data.description, creator_id: user?.id, thumbnail_url: thumbnailUrl, birth_year: Number(data.birth_year), running_time: Number(data.running_time), casts, staffs })
          .eq("id", videoId);
        if (error) throw error;
      } else {
        const id = v4();
        const { error } = await supabaseClient.from("videos").insert({ id, title: data.title, description: data.description, creator_id: user?.id, thumbnail_url: thumbnailUrl, birth_year: Number(data.birth_year), running_time: Number(data.running_time), casts, staffs });
        if (error) throw error;
        setVideoId(id);
      }
